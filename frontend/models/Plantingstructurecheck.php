...
//			// 农场区域
			$plantArea = [];
			foreach ($plantid as $id => $val) {
				$sum = Plantingstructurecheck::find()->where(['management_area'=>$area,'plant_id'=>$id,'year'=>User::getYear()])->sum('area');
				if($sum)
					$plantArea[] = (float)sprintf("%.2f", $sum);
			}
//			$result[] = $plantArea;
//		}
	...
	public static function getPlantGoodseedSum() {
		...
		$planting = Plantingstructurecheck::find()->where(['management_area'=>$area,'year'=>User::getYear()])->all();
