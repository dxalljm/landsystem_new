...
class Fireprevention extends \yii\db\ActiveRecord
{
	...
	public static function getAllbfb()
	{
		$i = 0;
		$finished = [];
		$allfire = 0;
...
// 		$amountsColor = ['#fedfdf','#feeedf','#fefddf','#e1fedf','#dffcfe','#dfe3fe','#fedffe'];
		...
		foreach ( Farms::getManagementArea()['id'] as $value ) {
			$allfire = $query->andFilterWhere(['management_area'=>$value,'year'=>User::getYear()])->count();
			$part = $query->andFilterWhere(['management_area'=>$value,'year'=>User::getYear(),'finished'=>1])->count();
		}
		if($allfire) {
			return (float)sprintf("%.2f", $part/$allfire);
		} else {
			return 0;
		}
	...
	public static function getData($total)
	{
		$i = 0;
		$all = [];
		$part = [];
...
//		var_dump($finished);
	...
	public static function getBfblist()
	{
		$i = 0;
		$all = [];
		$part = [];
