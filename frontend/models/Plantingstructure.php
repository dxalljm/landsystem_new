...
//     		var_dump($result);exit;
    		...
    		$result = Plantingstructure::find()->where(['farms_id'=>$farms_id,'lease_id'=>$lease_id,'year'=>User::getYear()])->sum('area');
    	}
	...
	public static function getWriteArea($farms_id)
	{
		$result = self::find()->where(['farms_id'=>$farms_id,'year'=>User::getYear()])->sum('area');
//		var_dump($result);exit;
		return sprintf("%.2f",$result);
	}
    ...
    {
    	$over = (float)self::getOverArea($lease_id, $farms_id);
//     	var_dump($over);exit;
...
//     	exit;
	...
	public static function getTypename($where)
	{
		$result = [];
		$data = Plantingstructure::find()->where($where)->all();
