...
//			$newdata = Farms::unique_arr($data);
	...
	public static function showFarminfo($farms_id)
	{
		...
		$html .= '<td width="26" rowspan="4" align="center">'.Html::img(Farmerinfo::find()->where(['cardid'=>$farm->cardid])->one()['photo'],['height'=>'130px']).'</td>';
		$html .= '</tr>';
		$html .= '<tr>';
...
//					$html .= '</tr>';
		...
		if($farm->notstateinfo == '' or $farm->notstateinfo == 0) {
            ...
            $html .= '<td width="23" align="left">'.Farms::getStateInfo($farm->notstateinfo).'</td>';
        }
...
//     	$sum = 0.0;
		...
		$result = ['area'=>['number'=>$areas,'percent'=>$percent],'row'=>['number'=>$rows,'percent'=>$rowpercent]];
//     	var_dump($areas);
		//$allvalue = $all - $sum;
...
//				var_dump('3333');
	...
	public static function getLastContractNubmer($parameter=NULL)
	{
		$number = [];
