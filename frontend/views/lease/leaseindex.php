...
<!--	--><?php
		?>
	<?php if(Plantingstructure::getWriteArea($_GET['farms_id']) < $farm['contractarea']) {
		if(User::disabled()) {
			echo Html::a('添加', '#', ['class' => 'btn btn-success','disabled'=>User::disabled()]);
		} else {
			echo Html::a('添加', ['leasecreate','farms_id'=>$_GET['farms_id']], ['class' => 'btn btn-success']);
		}
	} else {?>
	<p>
	...
	<?php 
		$plantings = Plantingstructure::find()->where(['farms_id'=>$_GET['farms_id']])->andWhere ( 'update_at>=' . Theyear::getYeartime ()[0] )->andWhere ( 'update_at<=' . Theyear::getYeartime ()[1] )->all();
		$sum = 0.00;
		if($plantings) {
...
//	var_dump($sum);
		if($overarea or $sum <= $farm['contractarea']) {
...
//					$action = $this->controller.'view';
                ...
                'plant' => function ($url, $model, $key) {
                	$over = (float)Plantingstructure::getOverArea($model->id, $_GET['farms_id']);
                	$leaseArea = (float)Plantingstructure::getLeaseArea($_GET['farms_id']);
                	if(!bccomp($over, $leaseArea)) {
                		return html::button('种植结构',['class' => 'btn btn-success btn-xs','disabled'=>true]);
                	} else {
                		return html::button('种植结构',['class' => 'btn btn-success btn-xs','onclick'=>'showModel('.$model->id.','.$_GET['farms_id'].')']);
                	}
                	return html::button('种植结构',['class' => 'btn btn-success btn-xs','onclick'=>'showModel('.$model->id.','.$_GET['farms_id'].')']);
                },
...
function showModel(lease_id,farms_id) {
    $.get('index.php?r=plantingstructure/plantingstructureleasecreate', {lease_id:lease_id,farms_id:farms_id}, function (body) {
		$('#dialogMsg').html(body);
