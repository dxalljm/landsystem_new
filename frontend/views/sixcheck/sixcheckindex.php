            ...
            <div class="col-md-4">
                <div class="card card-chart">
                    ...
                    <div class="card-content">
                        ...
                        <p class="category"><?php if(Plantingstructure::getWriteArea($_GET['farms_id']) < $farm['contractarea']) {if(User::disabled()) $distate=true;else $distate=false;} else {if(User::disabled()) $distate=true; else $distate=false;} echo Html::a('点此添加租赁信息','#',['onclick'=>'leasecreateDialog('.$_GET['farms_id'].')','class'=>'btn btn-xs','disabled'=>$distate])?></p>
                    </div>
                </div>
                            ...
                            <div class="card-content table-responsive" style="overflow-y:scroll;width:500px; height:180px;">
                                ...
                                if($plantingstructure) {
                                    ...
                                    foreach ($plantingstructure as $key => $value) {
                                        ...
                                        echo '<td>' . Plant::find()->where(['id'=>$value['plant_id']])->one()['typename'] .$goodseedHtml. '</td>';
                                        echo '<td>' . $value['area'] . '&nbsp;亩</td>';
                                        echo '<td>' .Html::a('投入品','#',['class'=>'btn btn-xs btn-success','onclick'=>'createT('.$value['id'].')','disabled'=>User::disabled()]).'</td>';
                                        echo '</tr>';
                                    }
                                    echo '</table>';
                                } else {
