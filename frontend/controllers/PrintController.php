...
// 		var_dump($lease);exit;
		...
		$insurance = Insurance::find()->where(['farms_id'=>$farm['id'],'lease_id'=>$lease_id,'year'=>User::getYear()])->one();
		if(empty($insurance)) {
			$insurance = Insurance::find()->where(['farms_id'=>$farm['id'],'lease_id'=>0,'year'=>User::getYear()])->one();
		}
// 		var_dump(html_entity_decode($this->formatName(date('Y年m月d日'),19)));

		$Plantingstructure = Plantingstructure::find()->where(['farms_id'=>$farm['id'],'year'=>User::getYear(),'lease_id'=>$lease['id']])->all();
...
//		}
		...
		$templateProcessor->setValue('year', htmlspecialchars(User::getYear()));
...
//					$plantings = Plantingstructure::find()->where(['lease_id'=>$lease_id,'plant_id'=>$typeid])->one();
		...
		$employee = Employee::find()->where(['farms_id'=>$farm['id'],'year'=>User::getYear()])->all();
// 		var_dump($employee[0]);
