...
//		$html .= '</table>';
	...
	public function getPlantingstructurecheck($begindate=null,$enddate=null)
	{
		...
		foreach ($plant as $key => $val) {
			$checks = Plantingstructure::find()->where(['management_area'=>$management_area['id'],'year'=>User::getYear()]);
			if(!empty($begindate)) {
				$checks->andFilterWhere(['between','update_at',$begindate,$enddate]);
			}
			$area = $checks->andFilterWhere(['plant_id'=>$key])->sum('area');
			if($area)
				$plantArea[] = ['name'=>Plant::findOne($key)->typename,'value' => (float)sprintf("%.2f", $area)];
		}
		$checks = Plantingstructure::find()->where(['management_area'=>$management_area['id'],'year'=>User::getYear()]);
		if(!empty($begindate)) {
			$checks->andFilterWhere(['between','update_at',$begindate,$enddate]);
...
// 		}
		...
		switch ($controller) {
			case 'farms' :
				...
				$value ['info'] = '<h2 class="card-title">'.$query->count ().'<span class="text-sm">户农场</span><br>'.$query->sum('contractarea').'<span class="text-sm">亩</span></h2>';
